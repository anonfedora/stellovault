...
async function fetchAccountBalances(publicKey: string): Promise<TokenBalance[]> {
  ...
  return SUPPORTED_TOKENS.map((token) => {
    if (token.issuer === null) {
      const raw = data.balances?.find((b) => b.asset_type === 'native');
...
export function useLiquidity(publicKey: string | null) {
   ...
   * Builds a standard Stellar payment as the on-chain action.
   */
  const executeOperation = useCallback(
    async (
      ...
      try {
        ...
        const asset =
          token.issuer === null ? Asset.native() : new Asset(token.code, token.issuer);

        // deposit: user → vault; withdraw: vault → user (simplified as self-payment for demo)
