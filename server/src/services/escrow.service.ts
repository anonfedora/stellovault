...
const ESCROW_ALLOWED_TRANSITIONS: Record<
  EscrowStatus,
  ReadonlySet<EscrowStatus>
> = {
  PENDING: new Set([
    "PENDING",
    "FUNDED",
...
function normalizeLiveEscrowStatus(value: unknown): EscrowStatus | "UNKNOWN" {
  const normalized = String(value ?? "")
    .trim()
    .toUpperCase();
  if (normalized === "ACTIVE") return "FUNDED";
...
export class EscrowService {
  ...
  async createEscrow(payload: CreateEscrowRequest) {
    ...
    if (!escrowContractId) {
      throw new Error(
        "Escrow contract ID not configured: contracts.escrow is empty",
      );
    }
    ...
    websocketService.broadcastEscrowCreated(
      escrow.id,
      escrow.buyerId,
      escrow.sellerId,
    );
  ...
  async getEscrowStatus(id: string) {
    const escrow = await this.getEscrow(id);
    const onChainReference = escrow.escrowAddress?.trim() || id;
    if (!/^\d+$/.test(onChainReference)) {
      throw new ValidationError(
        "Escrow does not have a numeric on-chain identifier to query",
      );
    }

    const liveEscrow = await contractService.getEscrowState(onChainReference);
    const liveStatus = normalizeLiveEscrowStatus(
      (liveEscrow as Record<string, unknown> | null)?.status,
    );

    return {
      escrowId: escrow.id,
      onChainEscrowId: onChainReference,
      databaseStatus: escrow.status,
      liveStatus,
      synchronized: liveStatus !== "UNKNOWN" && liveStatus === escrow.status,
      liveEscrow,
    };
  }
  ...
  async listEscrows(query: EscrowListQuery) {
    ...
    if (query.status?.trim())
      where.status = normalizeStatus(query.status, "status");
  ...
  async processEscrowEvent(event: EscrowEventPayload) {
    ...
    const allowedNextStatuses =
      ESCROW_ALLOWED_TRANSITIONS[existing.status as EscrowStatus];
